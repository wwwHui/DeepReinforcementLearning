...
class Config:  # for off-policy
    ...
    def init_before_training(self):
        if self.cwd is None:  # set cwd (current working directory) for saving model
            self.cwd = f'../outputs/{self.env_name}-{self.agent_class.__name__[5:]}'
...
class AgentDQN(AgentBase):
    ...
    def explore_env(self, env, horizon_len: int, if_random: bool = False) -> [Tensor]:
        ...
        masks = torch.zeros(horizon_len, dtype=torch.bool).to(self.device)
    ...
    def get_obj_critic(self, buffer, batch_size: int) -> (Tensor, Tensor):
        with torch.no_grad():
            state, action, reward, mask, next_state = buffer.sample(batch_size)
            next_q = self.cri_target(next_state).max(dim=1, keepdim=True)[0]
            q_label = reward + mask * self.gamma * next_q
...
class ReplayBuffer:  # for off-policy
    def __init__(self, max_size: int, state_dim: int, action_dim: int, gpu_id: int = 0):
        ...
        self.masks = torch.empty((max_size, 1), dtype=torch.float32, device=self.device)

    def update(self, items: [Tensor]):
        states, actions, rewards, masks = items
        ...
        if p > self.max_size:
            ...
            self.masks[p0:p1], self.masks[0:p] = masks[:p2], masks[-p:]
        else:
            self.states[self.p:p] = states
            self.actions[self.p:p] = actions
            self.rewards[self.p:p] = rewards
            self.masks[self.p:p] = masks
        self.p = p
        self.cur_size = self.max_size if self.if_full else self.p

    def sample(self, batch_size: int) -> [Tensor]:
        ids = torch.randint(self.cur_size - 1, size=(batch_size,), requires_grad=False)
        return self.states[ids], self.actions[ids], self.rewards[ids], self.masks[ids], self.states[ids + 1]
...
def train_agent(args: Config):
    ...
    while True:  # start training
        ...
        evaluator.evaluate_and_save(agent.act, args.horizon_len, exp_r, logging_tuple)
...
class Evaluator:
    ...
    def evaluate_and_save(self, actor, horizon_len: int, exp_r: float, logging_tuple: tuple):
